            ...
            if sentence.strip() != "":
                ...
                if (
                    ...
                    if TIMING and first_audio:
                        ...
                        print(f"\n\nin 22")
                        pattern = r"(Filler:.*?\.)(.*Filler:.*?\.)"
                        modified_sentence = re.sub(pattern, r"\1", clean_sentence)
                        print(f"modified_sentence: {modified_sentence}")
                        if modified_sentence:
                            clean_sentence = modified_sentence

                        filler = clean_sentence.split(".")[0]
                        rem_filler = re.sub(r"Filler:", "", filler)
                        filler = f"{rem_filler}"
                        print(f"rem_filler: {filler}")
                        if filler:
                            output1 = self.run_tts(filler)

                        rest_of_sentence = clean_sentence.split(".")[1:]
                        rest_of_sentence = "".join(rest_of_sentence)
                        print(f"rest_of_sentence: {rest_of_sentence}")
                        if rest_of_sentence:
                            output2 = self.run_tts(rest_of_sentence)

                    if type(output) == np.ndarray:
                        audio_queue.put((output, clean_sentence))
                    if type(output1) == np.ndarray:
                        audio_queue.put((output1, filler))
                    if type(output2) == np.ndarray:
                        audio_queue.put((output2, rest_of_sentence))
